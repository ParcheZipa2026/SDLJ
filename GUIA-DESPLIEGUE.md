...
1. En tu proyecto de Apps Script, pega **`Codigo-Apps-Script.gs`** y guarda.
2. Ejecuta una vez la función **`setupSpreadsheet`** (menú ▶) y autoriza los permisos.
   - Si el script está unido a una hoja, la usa.
   - Si es un script **independiente** (como el tuyo), crea la hoja de cálculo automáticamente y escribe su enlace en el **Registro de ejecución**. Ábrelo desde ahí; esa es tu base de datos.
   - En ambos casos crea las 10 hojas con sus encabezados, siembra eventos y responsables de ejemplo, y arma la carpeta raíz en Drive con la estructura de cada evento.
